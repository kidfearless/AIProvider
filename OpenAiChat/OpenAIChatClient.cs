...
internal sealed partial class OpenAIChatClient : IChatClient
{
    ...
    public OpenAIChatClient(ChatClient chatClient)
    {
        ...
        var providerUrl = typeof(ChatClient).GetField("_endpoint", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
            ?.GetValue(chatClient) as Uri ?? DefaultOpenAIEndpoint;
        var model = typeof(ChatClient).GetField("_model", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
        ...
        foreach (var content in contents)
        {
            switch (content)
            {
                ...
                case DataContent dataContent:
                    var binData = BinaryData.FromBytes(dataContent.Data);

                    parts.Add(ChatMessageContentPart.CreateFilePart(binData, dataContent.MediaType, Guid.NewGuid().ToString()));
                    break;
            }
        }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var update in updates.WithCancellation(cancellationToken).ConfigureAwait(false))
        {
            ...
            if (update.ToolCallUpdates is { Count: > 0 } toolCallUpdates)
            {
                ...
                {
                    functionCallInfos ??= [];
                    if (!functionCallInfos.TryGetValue(toolCallUpdate.Index, out var existing))
                    {
...
#pragma warning restore OPENAI001
    ...
    private static ChatTool ToOpenAIChatTool(AIFunction aiFunction)
    {
        ...
        var strict =
            !aiFunction.AdditionalProperties.TryGetValue("Strict", out var strictObj) ||
