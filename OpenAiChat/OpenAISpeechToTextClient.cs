...
internal sealed class OpenAISpeechToTextClient : ISpeechToTextClient
{
    ...
    public OpenAISpeechToTextClient(AudioClient audioClient)
    {
        ...
        var providerUrl = typeof(AudioClient).GetField("_endpoint", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
            ?.GetValue(audioClient) as Uri ?? _defaultOpenAIEndpoint;
        var model = typeof(AudioClient).GetField("_model", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
    ...
    private static AudioTranscriptionOptions ToOpenAITranscriptionOptions(SpeechToTextOptions? options)
    {
        ...
        if (options is not null)
        {
            ...
            if (options.AdditionalProperties is { Count: > 0 } additionalProperties)
            {
                ...
                if (additionalProperties.TryGetValue(nameof(result.TimestampGranularities), out var timestampGranularities))
                {
